        ...
        day.addEventListener('click', () => {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.filtered_delivery.length === 0) {
                        const filteredTableThead = $('#delivery-table-content thead');
                        ...
                        const filteredTable = $('#delivery-table-content tbody');
                        filteredTable.hide();
                    } else {
                        const filteredTableThead = $('#delivery-table-content thead');
                        filteredTableThead.empty();
                        ...
                        const row = $('<tr></tr>');
                        row.append($('<th>'));
                        row.append($('<th>').text('Dostawca'));
                        row.append($('<th>').text('Forma'));
                        row.append($('<th>').text('Ilość'));
                        ...
                        filteredTable.empty();
                        filteredTable.show();
                        for (const delivery of data.filtered_delivery) {
                            const row = $('<tr></tr>');

                            row.append($('<td>'));
                            row.append($('<td>').text(delivery.delivery_company));
                            row.append($('<td>').text(delivery.form));
                            row.append($('<td>').text(delivery.quantity));
                            row.append($('<td>').text(delivery.description.length > 70 ? delivery.description.substring(0, 70) + "..." : delivery.description));

                            if (delivery.status === 'W drodze') {
                                row.append($('<td class="primary">').text(delivery.status));
                            } else if (delivery.status === 'Odebrana') {
                                row.append($('<td class="success">').text(delivery.status));
                            } else if (delivery.status === 'Nie dostarczona') {
                                row.append($('<td class="danger">').text(delivery.status));
                            }

                            const detailsLinkCell = $('<td class="primary">');
                            const detailsLink = $('<a class="primary">').text('Szczegóły').attr('href', '/delivery/' + delivery.id + '/');
